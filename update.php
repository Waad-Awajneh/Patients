...
$query="SELECT * FROM `patients_info` where id=? ";
...
<form action="update.php" method="post">
  <div class="mb-3">
    ...
    <input type="text" id="name" class="form-control" name="name" value=<?php echo $patient->Name?>>
  </div>
  <div class="mb-3">
...
$query= $connect->prepare("UPDATE `patients_info` SET `Name` = ?, `Age` = ?, `Address` = ? WHERE `id` = ?");

$query->execute([$_POST['name'],$_POST['Age'],$_POST['Address'],$_POST['id']]);
